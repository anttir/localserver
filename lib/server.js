...
var deasync = require('deasync'); // allows async functions to be calles synchronously
...
(function (server) {
    ...
    server.loadSettings = function (setup, storage, argv) {
        ...
        if (server.serverAddr == "auto") {
            console.log('Aquire server IP automatically');
            var getIP = externalip(); // externalip()
            getIP(function (err, ip) {
                if (err) { throw err; } // every service in the list has failed 
                server.serverAddr = ip;
            });
            deasync.loopWhile(function () { return server.serverAddr == "auto"; });
            console.log('Server IP aquired');
        }
        ...
        console.log("server address: " + server.serverAddr);
